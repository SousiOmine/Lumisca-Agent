...
* **トークン**: 初回に生成され、以降の `install` では `service.env` の値を引き継ぎます（手動で編集した値も保持されます。値を消したい場合は行を削除します）。`service config` では伏せ字で表示されます。  
  ブラウザは `?token=` 付きの URL（`service install` / `status` が表示する「接続先」）を一度開けばトークンを Cookie に保存するため、2 回目以降はトークン無しの URL（例: `http://100.64.0.5:8000/`）で開けます。資格情報を持たないアクセスには案内ページ（401）を返します。
...
依存のバージョンは各マニフェストに固定されており、解決経路ごとに更新コマンドが異なります。手で書き換えず、次のコマンドで更新してください。`jsr:` / `npm:` のいずれも範囲指定（`^`）は使わず正確なバージョンを書き、`deno.lock` はその解決結果の記録です（`deno update` は両方を書き換えます）。
